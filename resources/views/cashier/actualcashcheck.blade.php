...
<div class='container'>
    ...
    <table class="table table-striped">
        <tr><td>Deposit Bank</td><td  align="right">Check Amount </td><td  align="right">Cash Amount</td><td  align="right">Total</td></tr>
    <?php
    $cashtotal = 0; $check1total=0; 
    ?>
    @if(count($chinabank)>0)
    @foreach($chinabank as $cb)
    <tr><td>China Bank</td><td align="right">{{$cb->checkamount}}</td><td  align="right">{{$cb->amount}}</td><td  align="right">{{$cb->amount+$cb->checkamount}}</td></tr>
    <?php
    ...
    @endforeach
    @else
    <tr><td>China Bank</td><td>0.00</td><td>0.00</td></tr>
    @endif
    @if(count($bpi1)>0)
    @foreach($bpi1 as $cb)
    <tr><td>BPI 1</td><td  align="right">{{$cb->checkamount}}</td><td  align="right">{{$cb->amount}}</td><td  align="right">{{$cb->amount+$cb->checkamount}}</td></tr>
    <?php
    $cashtotal=$cashtotal + $cb->amount;
    $check1total=$check1total + $cb->checkamount;        
    ?>
    @endforeach
    @else
    <tr><td>BPI 1</td><td>0.00</td><td>0.00</td></tr>
    @endif
    @if(count($bpi1)>0)
    @foreach($bpi2 as $cb)
    <tr><td>BPI 2</td><td  align="right">{{$cb->checkamount}}</td><td  align="right">{{$cb->amount}}</td><td  align="right">{{$cb->amount+$cb->checkamount}}</td></tr>
    <?php
    $cashtotal=$cashtotal + $cb->amount;
    $check1total=$check1total + $cb->checkamount;        
    ?>
    @endforeach
    @else
    <tr><td>BPI 2</td><td>0.00</td><td>0.00</td></tr>
    @endif
