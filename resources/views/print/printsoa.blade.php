...
<body> 
    ...
    <p align="center"><span style="font-size:14pt; font-weight: bold;">STATEMENT OF ACCOUNT</span><br>
     Date : {{date('M d, Y')}}</p>
   <table>
       <tr><td>Student No :</td><td colspan="2">{{$users->idno}}</td></tr>  
       <tr><td>Name :</td><td colspan="2">{{$users->lastname}}, {{$users->firstname}} {{$users->middlename}}</td></tr>
       @if(count($statuses)>0)
       <tr><td>Grade/Level : {{$statuses->level}}</td><td>Section : {{$statuses->section}}</td>
       <td>
           @if($statuses->level == "Grade 9" || $statuses->level == "Grade 10" || $statuses->level == "Grade 11" || $statuses->level == "Grade 12" )
           Strand/Shop : {{$statuses->strand}}
           @endif
       </td></tr>
       @endif
    </table>   
<table><tr><td width="70%" valign="top">
 ...
 @if(count($others)>0)
        ...
        $to = $to + $other->balance;
        ?>
        <tr><td>{{$other->transactiondate}}</td><td>{{$other->receipt_details}}</td><td align="right">{{number_format($other->balance,2)}}</td></tr>
        @endforeach
        <tr><td colspan="2">Total</td><td align="right"><?php echo number_format($to,2);?></td></tr>
        </table>
 @endif
</td>
</table>
